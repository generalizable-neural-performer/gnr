...
## Data Capture
GeneBody dataset captures performer in a motion capture studio with 48 synchronized cameras. Each actor is asked to perform 10 seconds clips recorded in a 15fps rate. The camera location and capture volume is visualized in the following video.
...
### Human labeling
We also choose 8 camera views to manually check the results of auto annoataion and manually labels the bad case. The 8 camera views are [01, 07, 13, 19, 25, 31, 37, 43].
...
### SMPLx parameters
GeneBody provide SMPLs parameter and 3D keypoints in `param` subfolder. More specifally, the dictionary 'smplx', can be directly feed to SMPLX [forward pass](https://github.com/vchoutas/smplx/blob/master/smplx/body_models.py#L1111).

### SMPLx scale
GeneBody has a large variation on performers' age distribution, while SMPLx model typically fails to fit well on kids and giants. We introduce 'smplx_scale' outside the SMPLx model, and jointly optimize scale and body model parameters during fitting. Thus, to recover the mesh using SMPLX model, you need to multiple a 'smplx_scale' to the vertices or 3d joints output by the body model.