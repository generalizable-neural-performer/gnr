...
def save_ply(fname, vertices, rgbs=None):
    fmt = '%.6f %.6f %.6f' if rgbs is None else '%.6f %.6f %.6f %d %d %d'
    header = f'ply\nformat ascii 1.0\nelement vertex {pts.shape[0]}\nproperty float x\nproperty float y\nproperty float z\nend_header' if rgbs is None else \
                f'ply\nformat ascii 1.0\nelement vertex {pts.shape[0]}\nproperty float x\nproperty float y\nproperty float z\nproperty uchar red\nproperty uchar green\nproperty uchar blue\nend_header'
    if rgbs is not None:
        vertices = np.concatenate([vertices, rgbs], -1)

    np.savetxt(fname, vertices, fmt=fmt, comments='', header=(header))
